...
function SliderArrow({ direction, onClick, disabled }) {
  const label = direction === "prev" ? "Previous related article" : "Next related article";
...
export default function RelatedBlogsSlider({ posts }) {
  ...
  const canSlide = posts.length > slidesPerView;
  ...
  return (
    <div className="home1-related-slider" aria-roledescription="carousel" aria-label="Related articles">
      <div className="home1-related-slider__stage">
        ...
        >
          ...
          >
            {posts.map((post, i) => (
              <li key={post.slug} className="home1-related-slider__slide" style={{ width: `${slidePct}%` }}>
                <BlogCard post={post} />
              </li>
            ))}
          </ul>
      ...
      {canSlide && maxIndex > 0 ? (
        <div className="home1-related-slider__dots" role="tablist" aria-label="Related article slides">
